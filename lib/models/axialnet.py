...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        self.qkv_transform = qkv_transform(in_planes, out_planes * 2, kernel_size=1, stride=1,
                                           padding=0, bias=False) # qkvtransform is just a nn.conv1d from utils.py
        ...
        # Position embedding
        self.relative = nn.Parameter(torch.randn(self.group_planes * 2, kernel_size * 2 - 1), requires_grad=True) # randomly initialize position encodings
    ...
    def forward(self, x):
        ...
        stacked_similarity = self.bn_similarity(stacked_similarity).view(N * W, 3, self.groups, H, H).sum(dim=1) #summed along 3, gives (N*W, groups, H, H) output
        #stacked_similarity = self.bn_qr(qr) + self.bn_kr(kr) + self.bn_qk(qk)
        # (N, groups, H, H, W)
        similarity = F.softmax(stacked_similarity, dim=3)
        sv = torch.einsum('bgij,bgcj->bgci', similarity, v) #attn*v 
        sve = torch.einsum('bgij,cij->bgci', similarity, v_embedding) #attn*v_embedding (position)
        stacked_output = torch.cat([sv, sve], dim=-1).view(N * W, self.out_planes * 2, H) #attn*V+ attn*v_E
        output = self.bn_output(stacked_output).view(N, W, self.out_planes, 2, H).sum(dim=-2) #yo in paper attn*V+ attn*v_E sum, output dim (N,W,out,H)
